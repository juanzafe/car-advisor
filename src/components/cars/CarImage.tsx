...
type ImageAction = { type: 'RESET' } | { type: 'SET_ANGLE_INDEX'; payload: number } | { type: 'SET_ROTATING'; payload: boolean } | { type: 'SET_COLOR_INDEX'; payload: number };
...
export const CarImage = ({ car }: { car: CarSpec }) => {
  ...
  useCarRotation(car, rotating, angleIndex, (index: number | ((prev: number) => number)) => {
  ...
  const toggleNextColor = () => {
    dispatch({ type: 'SET_COLOR_INDEX', payload: (colorIndex + 1) % carService.colorList.length });
  };
    ...
    <div className="relative group bg-slate-50 h-48 flex items-center justify-center overflow-hidden rounded-t-xl">
      ...
      <div className="absolute bottom-3 inset-x-0 flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
        <div className="flex items-center gap-1 bg-white/90 backdrop-blur-sm p-1.5 rounded-full shadow-lg border border-slate-200">
          <button 
            onClick={() => dispatch({ type: 'SET_ANGLE_INDEX', payload: (angleIndex - 1 + carService.angles.length) % carService.angles.length })} 
          ...
          <button 
            onClick={() => dispatch({ type: 'SET_ROTATING', payload: !rotating })} 
          ...
          <button 
            onClick={() => dispatch({ type: 'SET_ANGLE_INDEX', payload: (angleIndex + 1) % carService.angles.length })} 
            className="p-1.5 hover:bg-slate-100 rounded-full text-slate-700"
          >
            <RotateCw size={18} />
          </button>
